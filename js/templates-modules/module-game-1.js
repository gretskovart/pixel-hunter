...
const moduleGame1 = () => {
  const moduleGame1Template = getElementFromTemplate(`
    <header class="header">
      <button class="back">
  ...
  const inputs = form.querySelectorAll(`.visually-hidden`);

  const selectAnswersGame1Handlers = () => {
    const checkAnsweredInputs = Array.prototype.slice.call(inputs).filter((input) => input.checked === true);
